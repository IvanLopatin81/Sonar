...
Процедура Печать(ТабДок, Ссылка) Экспорт
	...
	Макет = Документы._ДемоЗаказПокупателя.ПолучитьМакет("Печать");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	_ДемоЗаказПокупателя.Ответственный,
	|	_ДемоЗаказПокупателя.СчетаНаОплату.(
	|		НомерСтроки,
	|		Счет
	|	)
	|ИЗ
	...
	ОбластьСчетаНаОплатуШапка = Макет.ПолучитьОбласть("СчетаНаОплатуШапка");
	ОбластьСчетаНаОплату = Макет.ПолучитьОбласть("СчетаНаОплату");
	...
	Пока Выборка.Следующий() Цикл
		...
		ТабДок.Вывести(ОбластьСчетаНаОплатуШапка);
		ВыборкаСчетаНаОплату = Выборка.СчетаНаОплату.Выбрать();
		Пока ВыборкаСчетаНаОплату.Следующий() Цикл
			ОбластьСчетаНаОплату.Параметры.Заполнить(ВыборкаСчетаНаОплату);
			ТабДок.Вывести(ОбластьСчетаНаОплату, ВыборкаСчетаНаОплату.Уровень());
		КонецЦикла;
